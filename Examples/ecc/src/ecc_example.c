...
/**
*********************************************************************************************************
*                                 ECC EXAMPLES FOR HT503X
*
* @brief  Initialize examples of ECC using library functions
*
* @param  None
*
* @retval None
...
#elif  defined  HT6x3x
    HT_CMU_ClkCtrl1Config(CMU_CLKCTRL1_ECCEN,ENABLE);           /*打开ECC模块时钟                 */
#endif
...
#elif  defined  HT6x3x
        HT_CMU_ClkCtrl1Config(CMU_CLKCTRL1_ECCEN,DISABLE);      /*关闭ECC模块时钟                 */
#endif
        return(TRUE);
    }
    else
...
//case 0
    strcpy((char *)SHA_m.u8,"qwertyuiopasdfghjklsdfhjknbsdjkfhwyer97234725wjnrlkjy879qwertyuiopasdfghjklsdfhjknbsdjkfhwyer97234725wjnrlkjy879");
                                                      /*初始化消息数组*/
...
/*
********************************************************************************************************
*                              在[1,n-1]范围内生成一个真随机数函数
*
*函数说明：RAND产生一个随机数，放在pRand指向的地址，并检验产生的随机数是否在[1,n-1]范围内，n为基点的阶
*
*入口参数：pRand  uint32_t *型变量,其指向对象存储随机大整数k: pRand[0]为低位
*          pECCn  uint32_t *型变量,其指向对象存储基点的阶n:pECCn[0]为低位
*          num    uint32_t  型变量,随机数的bit 长度
*
*返回参数：Bool   =TRUE 产生一个随机数在[1,n-1]范围内
*                 =FALSE  没有产生符合要求的随机数
********************************************************************************************************
*/
Bool Rand_output(uint32_t *pRand, uint32_t *pECCn, uint32_t Num)
{
    uint8_t i, LenWord;

    HT_CMU_ClkCtrl0Config(CMU_CLKCTRL0_ADESRADEN, ENABLE); // enable random module
    // 产生若干个随机数
    LenWord = Num/32;
    for(i=0;i<LenWord;i++)
    {
        HT_RAND->RANDSTR = 0x0000;
        HT_RAND->RANDSTR|=0x8004;
        while((HT_RAND->RANDSTR&0x0004)==0x0004);
        pRand[i]=HT_RAND->RANDDAT;
    }
    //判断随机数是否在[1, n-1]范围内
    if((TRUE==BigDatNotZero(pRand,LenWord))
        &&(BigDatCompare(pECCn, pRand, LenWord)))
    {
        return TRUE;
    }
    else
...
*                         ECDH_Init
*
*函数说明：生成ECDH密钥协商算法所需的一方的密钥对
*
*入口参数：pDx   uint32_t* 型变量，基点x坐标
*          pDy   uint32_t* 型变量，基点y坐标
*          pn    uint32_t* 型变量，基点的阶
*          pd    uint32_t* 型变量，私钥
*          pQx   uint32_t* 型变量，私钥x坐标
*          pQy   uint32_t* 型变量，私钥y坐标
*
*返回参数：无
***********************************************************************/
void ECDH_Init(uint32_t *pDx, uint32_t *pDy, uint32_t *pn, uint32_t *pd, uint32_t *pQx, uint32_t *pQy)
{
    uint8_t i;

    //生成本地私钥
    Rand_output(pd, pn, 256);
    //生成本地公钥
    for(i=0; i<8; i++)
    {
        HT_ECC->PXREG=*pDx++;
    ...
    HT_ECC->ECCSTA=0;
    HT_ECC->ECCCON=0x0001; //启动点乘运算
    while(0==(HT_ECC->ECCSTA&0x0001));//等待运算完成
    //读取本地公钥Q
    pQx += 7;
    pQy += 7;
    for(i=0;i<8;i++)
...
*函数说明：计算共享密钥
*
*入口参数：pA_d  uint32_t  型变量，A方私钥
*          pB_Qx uint32_t* 型变量，B方公钥x坐标
*          pB_Qy uint32_t* 型变量，B方公钥y坐标
*          pKx   uint32_t* 型变量，生成的共享密钥x坐标
*          pKy   uint32_t* 型变量，生成的共享密钥y坐标
*
*返回参数：无
***********************************************************************/
void ECDH_protocol_A(uint32_t *pA_d, uint32_t *pB_Qx, uint32_t *pB_Qy, uint32_t *pKx, uint32_t *pKy)
{
    uint8_t i;
...
#endif                                        /* This File Only support HT6x3x, HT502x */
